...
from model.constants.BasicConstants import MT_LINEAR_REGRESSION
...
def order_features_by_r_squared(the_dict) -> DataFrame:
    ...
    for the_column in list(the_dict.keys()):
        ...
        variable_dict['r-squared'].append(the_dict[the_column].get_adjusted_r_squared())
...
class Linear_Model(ModelBase):
    ...
    def __init__(self, dataset_analyzer):
        ...
        self.logger.debug("An instance of Linear_Model has been created.")

        # set the model type
        self.model_type = MT_LINEAR_REGRESSION

    # build model for a single column
    def build_model_for_single_column(self, the_target_column: str, the_variable_column: str) -> Linear_Model_Result:
        """
        ...
        the_result = self.fit_mlr_model(the_target_column=the_target_column,
    ...
    def fit_mlr_model(self, the_target_column: str, the_variable_columns: list) -> Linear_Model_Result:
        """
        ...
        the_variable_df = self.encoded_df[the_variable_columns]
        the_result = None

        # cast data to floats
        X = self.encoded_df[the_variable_columns].astype(float)

        # get the target series
        y = self.encoded_df[the_target_column]
    ...
    def build_mlr_for_single_comparison(self, the_target_column: str) -> dict:
        """
    ...
    def select_next_feature(self, the_target_column: str, current_features: list, ignore_features=None) -> dict:
        """
        ...
        for the_column in the_variable_list:
            ...
            if the_column not in (current_features + ignore_features):
                ...
                the_result[the_column] = self.fit_mlr_model(the_target_column=the_target_column,
